...
def grabPage(link, category):
    ...
    if response.status_code == 200:
        ...
        # print(article)
        paragrapghs = article.text().split('\n')
        content = ''
        for paragraph in paragrapghs:
            if paragraph.startswith('發稿時間'):
                break
            content = content + paragraph + '\n'
        # article 是 PyQueryCollection，paragraph 是 PyQueryObject
        # for paragraph in article.items():
        #     text = paragraph.text()
        #     if text.startswith('發稿時間'):
        #         break
        #     # paragraph[0] 是 HTMLElement，HTMLElement 和 TextElement 是網頁元素的最基礎
        #     # 以 <div>123</div>456<div>789</div> 而言
        #     # <div>123</div> 是 HTMLElement
        #     # 456 是 TextElement
        #     # <div>789</div> 是 HTMLElement
        #     # 請參考 https://www.w3schools.com/jsref/dom_obj_all.asp
        #     print(paragraph[0].tagName)
